...
const AUTH_API_BASE_URL = 'http://localhost:9101'
...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    const validateEmail = async () => {
      try {
        const res = await fetch(`${AUTH_API_BASE_URL}/api/auth/emails`, { signal: controller.signal })
        ...
        if (!cancelled) {
          setIsEmailRegistered(false)
          setErrors(prev => ({ ...prev, email: 'Please use a registered email or log in.' }))
        }
      }
    }
  ...
  const handleContinue = async (e) => {
    ...
    try {
      const res = await fetch(`${CHECKOUT_API_BASE_URL}/api/checkout`, {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        alert(text || 'Failed to save checkout details')
        return
      }
      navigate('/shipping', { state: { contact, shipping } })
    } catch {
      alert('Failed to contact checkout service')
    }
  }
