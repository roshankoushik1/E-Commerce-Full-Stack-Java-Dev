...
const OrdersSection = () => {
	...
	useEffect(() => {
		const loadOrders = async () => {
			const isLocalAdmin = localStorage.getItem('auth_token') === 'admin_token' && user?.id === 'admin';
			const fetchOptions = isLocalAdmin ? { auth: false } : { auth: true };
			const ORDERS_BASE = 'http://localhost:9105';
			const attempts = [
				{ path: '/orders', base: ORDERS_BASE },
				{ path: '/api/orders', base: ORDERS_BASE }
			];
			for (const attempt of attempts) {
				try {
					const data = await apiFetch(attempt.path, { ...fetchOptions, base: attempt.base });
					if (Array.isArray(data)) { setOrders(data); setLoading(false); return; }
				} catch {
