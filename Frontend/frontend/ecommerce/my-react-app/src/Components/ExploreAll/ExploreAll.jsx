...
const ExploreAll = () => {
  ...
  useEffect(() => {
    ...
    const loadMeta = async () => {
      ...
      setMetaError(null)
      try {
        const base = 'http://localhost:9110/api'
        const [catRes, brandRes, sizeRes] = await Promise.allSettled([
        ...
        if (catRes.status === 'fulfilled' && catRes.value.ok) {
          ...
          const normalized = arr
            ...
            .map(c => c.trim())
            .filter(c => c.length > 0)
            .filter(c => c.toLowerCase() !== 'all') // remove any backend 'All'
        ...
        if (sizeRes.status === 'fulfilled' && sizeRes.value.ok) {
          ...
          const arr = Array.isArray(data) ? data : []
          console.log('Sizes from backend raw:', arr) // Debug log
          const normalized = arr
            .map(s =>
              typeof s === 'string'
                ? s
                : (s && (s.size || s.code || s.label || s.name)) // include 'size'
            )
            .filter(Boolean)
          console.log('Normalized sizes:', normalized)
          if (!cancelled && normalized.length) {
            setAllSizes(Array.from(new Set(normalized)))
          }
        }
      } catch (e) {
