...
export const BillProvider = ({ children }) => {
  ...
  React.useEffect(() => {
    if (!user) { setBill(null); lastCartSignature.current=''; return }
    ...
    try {
      ...
      // Intentionally not auto-generating here (handled in cart effect)
    } catch { /* ignore */ }
  ...
  const generateBill = useCallback(async (_selectedIds, { offerCode, applyOffer } = {}) => {
    ...
    const path = `/bills/generate${qs.length?('?'+qs.join('&')):''}`
 
    try {
      // NOTE: backend derives current unbilled cart items. Do NOT send body (null serialized caused issues).
      ...
      const msg = e.message || ''
      setError(msg)
      if (!/detached entity/i.test(msg)) toast.error(msg || 'Bill generation failed')
      return null
    } finally { setLoading(false) }
  }, [user, cartItems, bill, computeCartSignature])
  ...
  }, [bill, cartItems, generateBill, computeCartSignature, loading, user])
  ...
  }, [bill, cartItems, computeCartSignature, loading, user])
